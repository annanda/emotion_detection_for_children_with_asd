...
def merge_features_with_annotation(file_features, list_file_name_emotions, feature_type):
    ...
    path_dataset = os.path.join(MAIN_FOLDER, 'dataset', 'video', f'{feature_type}')
    merged.to_csv(f"{path_dataset}/{file_name_suffix}.csv", index=False)


def merge_for_all_files(list_file_features, list_file_annotation, feature_type):
...
def call_merge_video_files(feature_type, list_file_annotation=list_file_annotation_emotions):
    ...
    """
    path_features_train = os.path.join(MAIN_FOLDER, 'features', 'video', f'{feature_type}')
    ...
    merge_for_all_files(list_file_features_train, list_file_annotation, feature_type)
