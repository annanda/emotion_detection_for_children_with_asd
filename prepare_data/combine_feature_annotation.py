...
path_main_folder = pathlib.Path(__file__).parent.parent.absolute()
...
def merge_features_with_annotation(file_features, list_file_name_emotions):
    ...
    path_dataset = os.path.join(path_main_folder, 'dataset', 'video', 'AU')
...
def call_merge_video_au(list_file_annotation):
    ...
    """
    path_features_train = os.path.join(path_main_folder, 'features', 'video', 'AU')
...
if __name__ == '__main__':
    path_annotation_emotions = os.path.join(path_main_folder, 'labels', 'emotion_zones', 'emotion_names')
